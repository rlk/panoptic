...
void main()
{
    ...
    vec3 water = vec3(0.1, 0.4, 0.7);
    vec3 land  = norm(sample_color(gl_TexCoord[0].xy), color.k0, color.k1).rgb;
    ...
    vec3 l = 2.0 * nl / (nl + nv) * land;
    vec3 s = nl * water + pow(rv, 64.0) * light;

    gl_FragColor = vec4(mix(l, s, step(var_k, 0.5)), 1.0);
}
